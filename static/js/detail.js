{
  ...
  function MusicPlot() {
      ...
      this.myData.forEach((d, i, dArraySelf) => {
        ...
        octaveList.forEach((dd, j, ddArraySelf) => {
          ...
          octaveData.forEach((ddd, k, dddArraySelf) => {
            ...
            dddObj["octavePoint"] =
              ddObj["octaveMain"] + (k - this.numHalfGaussian) * this.binSize;
            dddObj["count"] = 0;
        ...
        if (elRef !== undefined) {
          dSelected.values.forEach(dx => {
            ...
            if (elRef2 !== undefined) {
              ...
              let idx = 0;
              for (
                let k = -this.numHalfGaussian;
                k <= this.numHalfGaussian;
                ++k
              ) {
                let up =
                  elRef2["octaveData"][idx]["octavePoint"] -
                  elRef2["octaveMain"];
                up = up * up;
                const sigma = 0.15;
    ...
    this.initializePlot = csvRawData => {
      ...
      this.totalMusicTimeInSec =
        parseFloat(csvRawData[csvRawData.length - 1]["note_time"]) / 1000.0;

      // get global min and max octave and count values
      this.extrema = findExtrema(selectedData);
      this.extremaCached = {
        minOctave: this.extrema.minOctave,
      ...
      this.yScaleMirrored = d3
        .scaleLinear()
        .domain([0, this.extrema.maxCount])
        .range([this.height / 2, this.height]);

      // axis
      this.xAxis = d3.axisBottom(this.xScale).ticks(this.numOctave);
      ...
      this.myText = this.myGroups
        ...
        .attr(
          "transform",
          `translate(${this.totalWidth}, ${this.margin.top + this.height})`
        );
      ...
      //             "octavePoint"
      //             "count"
      this.plotCollection = this.myGroups
        .append("g")
        .attr("class", "plot")
        .attr("transform", `translate(${this.margin.left}, ${this.margin.top})`)
        .selectAll("path")
      ...
      this.plotCollectionMirrored = this.myGroups
        .append("g")
        .attr("class", "plot")
        .attr("transform", `translate(${this.margin.left}, ${this.margin.top})`)
        .selectAll("path")
      ...
      this.plotCollection
        .attr("d", (d, i) => {
          // define path details https://developer.mozilla.org/en-US/docs/Web/SVG/Tutorial/Paths
          let pathData = this.areaGenerator(d["octaveData"]);
          return pathData;
        })
        .attr("stroke", (d, i) => {
          // each line is assigned a predefined color
          if (d["octaveUpdated"]) {
            return pickColor(i);
          } else {
            return pickColor(i);
  ...
  function AnimationController() {
    ...
    this.update = timeStamp => {
      ...
      this.timeElapsedText.textContent = (
        this.timeElapsedInMillisec / 1000.0
      ).toFixed(1);

      this.timeIntervalInMillisec = timeStamp - this.timeOld;
      this.fpsText.textContent = (
        (1.0 / this.timeIntervalInMillisec) *
        1000.0
      ).toFixed(0);
    ...
    this.canPlayAnimation = () => {
      if (
        this.plotRefreshAccumulatedTimeInMillisec >
        this.plotRefreshTimeInMillisec
      ) {
        this.plotRefreshAccumulatedTimeInMillisec = 0.0;
        return true;
      } else {
